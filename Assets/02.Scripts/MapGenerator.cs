...
public class MapGenerator : MonoBehaviour
{
    ...
    public void GenerateMap()
    {
        ...
        for (int i = 0; i < obstacleCount; i++)
        {
            ...
            if (randomCoord != mapCentre && MapIsFullyAcccessible(obstacleMap, currentObstacleCount))
            {
        ...
        Transform maskBottum = Instantiate(navmeshMaskPrefab, Vector3.back * (mapSize.y + maxMapSize.y) / 4 * tileSize, Quaternion.identity) as Transform;
        maskBottum.parent = mapHolder;
        maskBottum.localScale = new Vector3(maxMapSize.x, 1, (maxMapSize.y - mapSize.y) / 2) * tileSize;

        navmeshFloor.localScale = new Vector3(maxMapSize.x, maxMapSize.y) * tileSize;

    }

    bool MapIsFullyAcccessible(bool[,] obstacleMap, int currentObstacleCount)
    {
